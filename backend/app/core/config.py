...
class Settings(BaseSettings):
    ...
    alert_events_channel: str = Field(
        default="georisk:alerts:events", alias="ALERT_EVENTS_CHANNEL"
    )
    alert_ingestion_dead_letter_queue_key: str = Field(
        default="georisk:ingestion:alerts:dlq",
        alias="ALERT_INGESTION_DLQ_KEY",
    )
    alert_ingestion_max_retries: int = Field(
        default=3, alias="ALERT_INGESTION_MAX_RETRIES"
    )
    ...
    refresh_cookie_name: str = Field(
        default="grm_refresh_token", alias="REFRESH_COOKIE_NAME"
    )
